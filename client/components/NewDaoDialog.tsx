...
import { DEFAULT_TASKS, type Dao, type TeamMember } from "@shared/dao";
...
export default function NewDaoDialog({
  ...
  // Fetch the next DAO number from the server when dialog opens
  useEffect(() => {
    if (open) {
              ...
              <Select
                ...
                onValueChange={(value) => {
                  if (value === "none") {
                    ...
                    const leader = SAMPLE_TEAM_MEMBERS.find(
                      (m) => m.id === value,
                    );
                    if (leader) {
