...
router.get('/me', authenticate, asyncHandler(async (req: any, res: express.Response) => {
  res.json({ user: req.user });
}));

// GET /api/auth/users - Get all users (admin only)
router.get('/users', authenticate, requireAdmin, asyncHandler(async (req: express.Request, res: express.Response) => {
  try {
...
router.post('/users', authenticate, requireAdmin, asyncHandler(async (req: express.Request, res: express.Response) => {
  try {
    ...
    const result = await AuthServiceMongo.createUser({ name, email, role: role as UserRole });
...
router.put('/users/:id/role', authenticate, requireAdmin, asyncHandler(async (req: express.Request, res: express.Response) => {
  try {
    const { id } = req.params;
    const { role } = req.body;
    ...
    const updatedUser = await AuthServiceMongo.updateUserRole(id, role as UserRole);
    if (!updatedUser) {
...
router.delete('/users/:id', authenticate, requireAdmin, asyncHandler(async (req: any, res: express.Response) => {
  try {
    const { id } = req.params;
...
router.post('/change-password', authenticate, asyncHandler(async (req: any, res: express.Response) => {
  try {
    ...
    const success = await AuthServiceMongo.changePassword(req.user.id, newPassword);
    if (!success) {
      return res.status(404).json({ error: 'User not found' });
    }
...
router.put('/profile', authenticate, asyncHandler(async (req: any, res: express.Response) => {
  try {
    ...
    const updatedUser = await AuthServiceMongo.updateProfile(req.user.id, { name, email });
    if (!updatedUser) {
      return res.status(404).json({ error: 'User not found' });
    }
...
router.post('/verify-reset-token', asyncHandler(async (req: express.Request, res: express.Response) => {
  try {
...
router.post('/reset-password', asyncHandler(async (req: express.Request, res: express.Response) => {
  try {
    ...
    const success = await AuthServiceMongo.resetPasswordWithToken(token, email, newPassword);
    if (!success) {
      return res.status(400).json({ error: 'Code invalide ou expiré' });
    }
